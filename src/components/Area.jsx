...
const Area = ({area, country, timezone}) => {
    ...
    return (
        ...
        bg-gray-50 rounded bg-opacity-30 backdrop-filter backdrop-blur-md shadow-md dark:bg-gray-800 dark:bg-opacity-40">
            <h1 className="text-center text-xl lg:text-4xl tracking-widest uppercase">{area} / {country}</h1>
            <p className="text-sm md:text-md">{month}/{date} {weekDayArr[time.getUTCDay()]} {hours}:{mins}:{sec}</p>
        </div>
